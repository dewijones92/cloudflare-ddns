...
def deleteEntries(type):
    ...
    try:
        ...
        print(f"😡 Exception in deleteEntries for type '{type}': {e}")
        ...
        if ipv4_enabled:
            try:
                ...
                if not shown_ipv4_warning:
                    ...
                    print(f"🧩 IPv4 not detected via 1.1.1.1, trying 1.0.0.1. Error: {e}")
                ...
                try:
                    ...
                    if not shown_ipv4_warning_secondary:
                        ...
                        print(f"🧩 IPv4 not detected via 1.0.0.1. Error: {e}")
        ...
        if ipv6_enabled:
            try:
                ...
                if not shown_ipv6_warning:
                    ...
                    print(f"🧩 IPv6 not detected via 1.1.1.1, trying 1.0.0.1. Error: {e}")
                try:
                    aaaa = requests.get(
                    ...
                    if not shown_ipv6_warning_secondary:
                        ...
                        print(f"🧩 IPv6 not detected via 1.0.0.1. Error: {e}")
                        print("🧩 Verify your ISP or DNS provider isn't blocking Cloudflare's IPs.")
                if purgeUnknownRecords:
                    deleteEntries("AAAA")
        ips = {}
...
def commitRecord(ip):
    ...
    try:
        ...
        print(f"😡 Exception in commitRecord: {e}")
...
def updateLoadBalancer(ip):
    try:
        ...
        print(f"😡 Exception in updateLoadBalancer: {e}")
...
def cf_api(endpoint, method, config, headers={}, data=False):
    try:
        ...
        print(f"😡 An exception occurred while sending '{method}' request to '{endpoint}': {e}")
        return None
...
def updateIPs(ips):
    try:
        ...
        print(f"😡 Exception in updateIPs: {e}")
...
if __name__ == '__main__':
    ...
    if config is not None:
        try:
            ...
            ipv4_enabled = True
            print(f"⚙️ Error reading 'a' from config.json: {e}")
        ...
        try:
            ...
            ipv6_enabled = True
            print(f"⚙️ Error reading 'aaaa' from config.json: {e}")
            print("⚙️ Individually disable IPv4 or IPv6 with new config.json options. Read more about it here: https://github.com/timothymiller/cloudflare-ddns/blob/master/README.md")
        try:
            purgeUnknownRecords = config["purgeUnknownRecords"]
        except Exception as e:
            purgeUnknownRecords = False
            print(f"⚙️ No config detected for 'purgeUnknownRecords' - defaulting to False. Error: {e}")
        try:
            ttl = int(config["ttl"])
        except Exception as e:
            ttl = 300  # default Cloudflare TTL
            print(f"⚙️ No config detected for 'ttl' - defaulting to 300 seconds (5 minutes). Error: {e}")
        if ttl < 30:
            ttl = 1
    ...
    if (len(sys.argv) > 1):
        if (sys.argv[1] == "--repeat"):
            if ipv4_enabled and ipv6_enabled:
                print(f"🕰️ Updating IPv4 (A) & IPv6 (AAAA) records every {str(ttl)} seconds")
            elif ipv4_enabled and not ipv6_enabled:
                print(f"🕰️ Updating IPv4 (A) records every {str(ttl)} seconds")
            elif ipv6_enabled and not ipv4_enabled:
                print(f"🕰️ Updating IPv6 (AAAA) records every {str(ttl)} seconds")
            next_time = time.time()
            killer = GracefulExit()
            prev_ips = None
            while True:
                try:
                    ...
                    print(f"😡 Exception in main loop during update: {e}")
        ...
        try:
            ...
            print(f"😡 Exception during single update: {e}")
